import {extent1} from './vector';
import {complex} from './complex2';
...
//		}
	...
	roots(): number[] {
		if (this.c.length > 5) {
			const roots = aberth(this);
			return roots.filter(r => Math.abs(r.i) < epsilon).map(r => r.r).sort((a, b)=> a - b);
		}

		if (this.c.length === 2)
			return [-this.c[0] / this.c[1]];
		return normPolyRoots(this.c);
	}
}
/*
export function monomial(c: number) {
	return new normalised_polynomial([c]);
}
*/
//-----------------------------------------------------------------------------
...
function normPolyRoots(k: number[]): number[] {
	switch (k.length) {
		case 1:
			return [-k[0]];
		...
		case 4: {
			//biquadratic case
			if (k[1] === 0 && k[3] === 0) {
				const r = normPolyRoots([k[0], k[2]]).map(r => Math.sqrt(r));
				return r.length === 1
					? [-r[0], r[0]]
					: [-r[1], -r[0], r[0], r[1]];
			}
			...
			if (Math.abs(t) < epsilon) {
				...
				return [0, ...normPolyRoots([q, p, 0]).map(r => r - a / 4)];
			}

			// solve the resolvent cubic ...
			const r = normPolyRoots([-q * q / 8, p * p / 4 - t, p]);
			// ... and take the one real solution to build two quadratic equations
			const m = r[0];
			const v = Math.sqrt(m * 2);
			const u = q / Math.sqrt(m * 8);
			
			const ra = normPolyRoots([m + p / 2 - u,  v]);
			const rb = normPolyRoots([m + p / 2 + u, -v]);
