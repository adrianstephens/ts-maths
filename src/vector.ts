...
Object.assign(float2.prototype, ownProps(ops.prototype), {
	...
	[Symbol.for("debug.description")](this: float2)	{ return `(${this.x}, ${this.y})`; }
});
...
export class circle {
	constructor(public centre: float2, public r: number) {}
	public evaluate(angle: number) {
		return this.centre.add(float2.cossin(angle * Math.PI * 2).mul(this.r));
	}
}
...
export class statistics<T extends ops<T> & WithZero<T>> {
	...
	add(v: T) {
		...
		this.sum2 = this.sum2.add(v.vmul(v));
		this.count++;
		return this;
	}
	get mean() {
