...
exports.factorisationB = exports.factorisation = void 0;
exports.isProbablePrime = isProbablePrime;
exports.isProbablePrimeB = isProbablePrimeB;
...
function MillerRabin(n) {
    ...
    const d = t >> s;
    ;
    function tryWitness(a) {
        // quick gcd check: if a shares a factor with n, return it
        const factor = integer_1.default.gcd(a, n);
        if (factor > 1 && factor < n)
            return factor;
        let x = integer_1.default.modPow(a, d, n);
        if (x === 1 || x === n - 1)
            return 0;
        for (let r = 1; r < s; r++) {
            const x0 = x;
            ...
            if (x === 1) {
                ...
                const factor = integer_1.default.gcd(x0 - 1, n);
                if (factor > 1 && factor < n)
                    return factor;
                // try the other sign as well (rarely useful here, but harmless)
                const factor2 = integer_1.default.gcd(x0 + 1, n);
        ...
        return 1; //not prime, no factor found
    }
    for (const a of MR64) {
...
function pollardRhoBrent(n, f, y) {
    const m = 32;
    let y0 = y, x = y, g = 1;
    ...
    if (g === n) {
        ...
        for (let attempts = 0; g === 1 && attempts < 4096; attempts++) {
            y0 = f(y0);
            g = integer_1.default.gcd(Math.abs(x - y0), n);
        }
    }
    return g === 1 ? n : g;
}
...
*/
class factorisation extends Array {
    constructor(n) {
        if (Array.isArray(n)) {
            super(...n);
            return;
            ...
            if (factor) {
                if (factor === 1) {
                    for (let attempt = 0; attempt < 10; attempt++) {
                        const c = Math.random() * (m - 1) + 1;
                        // f(x) = x^2 + c mod n
                        factor = pollardRhoBrent(m, (x) => (x * x + c) % m, Math.random() * m);
                        if (factor < m)
                            break;
                    }
                }
    ...
    *divisors(limit) {
    ...
    pow(exp) {
        return new factorisation(this.map(f => f * exp));
    }
}
exports.factorisation = factorisation;
//------------------------ BigInt version -----------------------
const MR64B = [2n, 325n, 9375n, 28178n, 450775n, 9780504n, 1795265022n];
function makeWitnesses(n, rounds = 12) {
    const bits = n.toString(2).length;
    return (function* () {
        for (let i = 0; i < rounds; i++)
            yield big_1.default.random(bits) % (n - 3n) + 2n;
    })();
}
function isProbablePrimeB(n, witnesses = 12) {
    if (n < 4n)
        return n >= 2n;
 ...
 */
function MillerRabinB(n, witnesses) {
    // write n-1 as d * 2^s
    let d = n - 1n;
    let s = 0;
    /*
    ...
    */
    ...
    function tryWitness(a) {
        // quick gcd check: if a shares a factor with n, return it
        const factor = big_1.default.gcd(a, n);
        if (factor > 1n && factor < n)
            return factor;
        let x = big_1.default.modPow(a, d, n);
        if (x === 1n || x === n - 1n)
            return 0n;
        for (let r = 1; r < s; r++) {
            const x0 = x;
            ...
            if (x === 1n) {
                // Found nontrivial square root: x is such that x^2 ≡ 1 (mod n) but x ≠ ±1
                const factor = big_1.default.gcd(x0 - 1n, n);
                if (factor > 1n && factor < n)
                    return factor;
                // try the other sign as well (rarely useful here, but harmless)
                const factor2 = big_1.default.gcd(x0 + 1n, n);
...
*/
// find a factor of n using Pollard-Rho with Brent's cycle detection
function pollardRhoBrentB(n, f, y) {
    const m = 32;
    let y0 = y, x = y, g = 1n;
    for (let r = 1; g === 1n && r < 0x10000; r <<= 1) {
        ...
        for (let k = 0; k < r && g === 1n; k += m) {
            ...
            for (let i = 0; i < m && i < r - k; i++) {
                y = f(y);
                q = (q * big_1.default.abs(x - y)) % n;
            }
            g = big_1.default.gcd(q, n);
        }
    }
    if (g === n) {
        g = 1n;
        for (let attempts = 0; g === 1n && attempts < 4096; attempts++) {
            y0 = f(y0);
            g = big_1.default.gcd(big_1.default.abs(x - y0), n);
        }
    }
    return g === 1n ? n : g;
}
...
*/
class factorisationB extends Map {
    constructor(n) {
        if (typeof n !== 'bigint') {
            super(n);
            return;
        ...
        while (stack.length) {
            ...
            if (factor) {
                if (factor === 1n) {
                    const bits = m.toString(2).length;
                    for (let attempt = 0; attempt < 10; attempt++) {
                        const c = big_1.default.random(bits) % (m - 1n) + 1n;
                        //factor	= pollardRhoOnce(m, c);
                        // f(x) = x^2 + c mod n
                        factor = pollardRhoBrentB(m, (x) => (x * x + c) % m, big_1.default.random(bits) % m);
                        if (factor < m)
                            break;
                    }
                }
...
exports.factorisationB = factorisationB;
