...
const toBI = (n) => BigInt(n);
(0, test_1.test)('pollard: basic composites and factoring', () => {
    ...
    for (const c of composites) {
        ...
        const factors = new prime_1.factorisationB(n);
        let prod = 1n;
        for (const [p, e] of factors.entries())
            for (let i = 0; i < e; i++)
                prod *= p;
        (0, test_1.assert)(prod === n, `factorBigInt did not reconstruct ${c}: got ${prod}`);
        // ensure factorization produces at least two prime factors (or a repeated factor)
        const primesFound = Array.from(factors.keys());
        (0, test_1.assert)(!(primesFound.length === 1 && primesFound[0] === n), `expected composite factors for ${c}`);
    }
});
(0, test_1.test)('pollard: known primes', () => {
    const primes = [2, 3, 101, 103, 1009];
    for (const p of primes) {
        const f = new prime_1.factorisationB(toBI(p));
        // should factor to itself with exponent 1
        (0, test_1.assert)(f.size === 1, `expected single factor for prime ${p}`);
        const [prime] = Array.from(f.keys());
        (0, test_1.assert)(prime === toBI(p) && f.get(prime) === 1, `unexpected factor map for ${p}`);
    }
});
(0, test_1.test)('MR opportunistic extraction via MR64 base gcd', () => {
    ...
    (0, test_1.assert)(factors.size === 2, `expected two prime factors, got ${factors.size}`);
    (0, test_1.assert)(factors.get(bigFactor) === 1, `expected exponent 1 for ${bigFactor}`);
    (0, test_1.assert)(factors.get(other) === 1, `expected exponent 1 for ${other}`);
    // Multiply back to ensure full reconstruction
    let prod = 1n;
    for (const [p, e] of factors.entries())
        for (let i = 0; i < e; i++)
            prod *= p;
    (0, test_1.assert)(prod === n, `reconstructed product ${prod} !== n ${n}`);
});
console.log('test-mr-extraction OK');
//# sourceMappingURL=test-prime.js.map